...
drop function f_insert_data(u_count integer, tax float, u_id integer);
...
--//-------------------------------------------//--
create or replace procedure delete_data(u_count integer, u_id integer)
language  'plpgsql'
as $$
	BEGIN
	IF u_count = 0 THEN
...
drop procedure delete_data(u_count integer, u_id integer);

select * from products;

call delete_data(0, 1);
